...
# PROMPT = "Chinese: [source_sentence] English:"
# CHAT_PROMPT = "\"[source sentence]\" translate the above sentence to {tgt_lang}, and only return the content translated. no explanation."
# SYSTEM_MESSAGE = "You are a professional {src_lang}-to-{tgt_lang} translator."
CHAT_PROMPT = "[source sentence]\n\nTranslate to {tgt_lang}"
SYSTEM_MESSAGE = "You are a machine translation system that translates sentences from {src_lang} to {tgt_lang}. You just respond with the translation, without any additional comments."

def del_endswith_none(str1: str):
    """
    删除字符串首尾的空字符（空格、换行）并切分换行符只取第一行作为结果
    :param str1:
    """
    s = str1
    if str1 is not None:
...
def main(
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        generate_params = {
            ...
            # "eos_token_id": tokenizer.convert_tokens_to_ids(["<0x0A>"])[0],
        ...
        for k in range(len(outputs)):
            res.append(del_endswith_none(tokenizer.decode(outputs[k][len(inputs.input_ids[k]):], skip_special_tokens=True).strip()))
    ...
    for i in tqdm(range(step_size + 1)):
        ...
        if existed_results is not None:
            ...
            if skip:
                continue
        batch_data = test_data[start_idx: end_idx]['src']
        if len(batch_data) == 0:
            break
        outputs = evaluate(batch_data)
        for k in range(len(outputs)):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lang-pair', default=None, type=str, required=True,
    ...
    CHAT_PROMPT = CHAT_PROMPT.format_map({"tgt_lang": mapping[tgt]})
    SYSTEM_MESSAGE = SYSTEM_MESSAGE.format_map({"src_lang": mapping[src], "tgt_lang": mapping[tgt]})
