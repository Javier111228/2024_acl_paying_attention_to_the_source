...
class DataTrainingArguments:
    ...
    add_eos_token: bool = field(
        default=True, metadata={"help": "add </s> token to the end of sentence"}
    )
    ...
    prompt_name: Optional[str] = field(
        default="alpaca", metadata={"help": "The name of the prompt template to use."}
    )    
    lang_pair: Optional[str] = field(
        default="de-en", metadata={"help": "The translation direction."}
    )
...
def train():
    ...
    if model_args.model_name_or_path:
        ...
        model = AutoModelForCausalLM.from_pretrained(
            ...
            quantization_config=bnb_config_8bit if model_args.load_in_bits == 8 else (bnb_config_4bit if model_args.load_in_bits == 4 else None),
    ...
    prompter = Prompter(data_args.prompt_name)
    ...
    src_lang, tgt_lang = data_args.lang_pair.split('-')
    chat_prompt = CHAT_PROMPT.format(TGT = mapping[tgt_lang])
    # chat_prompt = CHAT_PROMPT
    if SYSTEM_MESSAGE:
        system_message = SYSTEM_MESSAGE.format(SRC = mapping[src_lang], TGT = mapping[tgt_lang])
    else:
        system_message = None
    ...
    def generate_and_tokenize_prompt(data_point):
        full_prompt = prompter.generate_prompt(
            chat_prompt.replace('[source sentence]', data_point["instruction"]),
