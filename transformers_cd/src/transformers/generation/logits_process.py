...
class ContrastiveAttentionLogitsProcessor(LogitsProcessor, LogitsWarper):
    r"""
    [`LogitsWarper`] and [`LogitsProcessor`] for normalizing the contrastive attention scores using log-softmax. It's important to normalize
    the scores during beam search, after applying the logits processors or warpers, since the search algorithm used in
    this library doesn't do it (it only does it before, but they may need re-normalization) but it still supposes that
    the scores are normalized when comparing the hypotheses.
    ...
    ```
    """
    def __init__(self, contrastive_lambda: int):
        self.contrastive_lambda = contrastive_lambda

    @add_start_docstrings(LOGITS_PROCESSOR_INPUTS_DOCSTRING)
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        convention_log_prob = scores[0].log_softmax(dim=-1)
        contrastive_log_prob = scores[1].log_softmax(dim=-1)
        scores = convention_log_prob - self.contrastive_lambda * contrastive_log_prob
        return scores
