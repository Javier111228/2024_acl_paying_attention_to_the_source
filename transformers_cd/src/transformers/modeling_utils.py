    ...
    def from_pretrained(
        ...
        * `low_cpu_mem_usage` algorithm:
        ...
        if from_pt:
            ...
            # add contrastive attn keys
            for item in loaded_state_dict_keys:
                    # 检查条目是否包含 'self_attn'
                if 'self_attn' in item:
                    # 创建新的条目名，替换 'self_attn' 为 'contrastive_attn'
                    new_item = item.replace('self_attn', 'contrastive_attn')
                    # 将新条目添加到 new_list 中
                    new_loaded_state_dict_keys.append(new_item)
            loaded_state_dict_keys = new_loaded_state_dict_keys
    ...
    def _load_pretrained_model(
        ...
        if state_dict is not None:
            ...
            for shard_file in resolved_archive_file:
                ...
                # copy self_attn weights to contrastive_attn_weights
                new_entries = {}
                for key in state_dict.keys():
                    # 检查键是否包含 'self_attn'
                    if 'self_attn' in key:
                        # 创建新的键名，替换 'self_attn' 为 'contrastive_attn'
                        new_key = key.replace('self_attn', 'contrastive_attn')
                        # 复制值到新键
                        new_entries[new_key] = state_dict[key].clone()
